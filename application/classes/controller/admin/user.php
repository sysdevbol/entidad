...
class Controller_Admin_User extends Controller_AdministratorTemplate {
   protected $user;
    protected $menus;
 ...
 public function after() {
        $this->template->menutop = View::factory('templates/menutopadmin')->bind('menus',$this->menus)->set('controller', 'user');
        $oSM=New Model_menus();
    ...
    public function action_resetPass(){
        ...
        $info=array();   
            ...
            if($_POST['pass2']==$_POST['pass1']){
                            ...
                            $user->password=$auth->hash_password($_POST['pass2']);
                            $user->save();                        
                            $info[]='Su contraseña fue  cambiado correctamente';
                            //vitacora
                            $this->save($this->user->id_entidad,$id, $this->user->nombre.' <b>'.$this->user->cargo.'</b> Cambio su contrasena de :'.$user->nombre); 
                        
            $this->request->redirect('/admin/user/detalle/'.$id);
            }
            else{
    ...
    public function action_list()
    {
        ...
        $this->template->descripcion.='Lista general de usuarios ';
        $this->template->scripts    = array('media/js/jquery.tablesorter.min.js');
        $this->template->content=View::factory('admin/users')
                                ->bind('users', $usuarios);
    }
