...
//if()
{
    ...
    while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
        ...
        //$pdf->Row(array());
        $pdf->MultiCell(180, 5, utf8_decode("El presente documento certifica la habilitación al Proveedor ".strtoupper($rs->nombre_proponente).". Representada por el Sr(a). ".strtoupper($rs->nombres_representante)." ".strtoupper($rs->paterno_representante)." ".strtoupper($rs->materno_representante)." para la participación en procesos de aprovisionamiento de materiales para distintas proyectos dependientes de la Agencia Estatal de Vivienda, con todos los derechos y que ello conlleva."), 0, 'J');
        $pdf->Ln(10);
       ...
       //codigo de barras____________________
        ...
        $pdf->Output('proveedor' . $rs->pin_empresa . '.pdf', 'D');
    }
}
